...
int main( int argc, char** argv )
{
  ...
  float dropoffXpos = -1.0;
  float dropoffYpos = 0.0;
  float dropoffZrot = 1.0;
...
*/
  ...
  marker.pose.position.x = pickupXpos;
  marker.pose.position.y = pickupYpos;
  marker.pose.position.z = 0;
  marker_pub.publish(marker);
  ...
  if (! goToPositionClient.call(goToPositionService)) {
    ...
    if (! goToPositionClient.call(goToPositionService)) {
      ...
      marker.action = visualization_msgs::Marker::ADD;
      marker.pose.position.x = dropoffXpos;
      marker.pose.position.y = dropoffYpos;
      marker.pose.position.z = 0;
      marker_pub.publish(marker);
